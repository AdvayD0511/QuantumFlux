...
def home():
    ...
    <head>
        ...
        <style>
            ...
            #map {width:100%; height:88vh; border-top:2px solid #00d8ff; border-bottom:2px solid #00d8ff; box-shadow:0 0 20px rgba(0,216,255,0.3); min-height: 400px;}
    ...
    <body>
        ...
        <script>
            ...
            async function loadMap(){
                console.log("Starting loadMap at", new Date().toLocaleTimeString());
                try {
                    const res = await fetch('/stations');
                    const stations = await res.json();
                    console.log("Received stations data:", stations.length, "items");

                    const heatPoints = stations
                        .filter(s => s.aqi !== null && s.aqi !== undefined && s.lat && s.lon)
                        .map(s => [s.lat, s.lon, Math.min(s.aqi/300, 1)]);
                    console.log("Heat points generated:", heatPoints.length, "points");
                    ...
                    if (heatPoints.length > 0) {
                        ...
                        console.log("No valid heat points to display.");
                    }
                    ...
                    console.error("loadMap failed:", error);
                    alert("Map data failed to load. Check console for details.");
                }
            }
...
def stations():
    ...
    try:
        ...
        print("Fetched", len(stations), "stations")
...
def get_pm25():
    ...
    if lat is not None and lon is not None:
        try:
            ...
            print("Fetching OWM data from:", owm_url)  # Debug print
